#12_formose_crn_case_study.jl
#this file applies the crn parser and lux message passing pipeline to a small formose-inspired crn case study
#the goal is to test the same workflow on another chemistry-style reaction network
#this is still a simplified formose-inspired example, not the final full literature network
...
println("\nformose-inspired crn reactions:")
...
formose_dh = DirectedHypergraph(tail_matrix, head_matrix)

println("\nDirectedHypergraph object type:")
println(typeof(formose_dh))
...
try
    components = get_weakly_connected_components(formose_dh)
    println(components)
catch err
...
#storing the formose case study data

formose_case_study_data = (
    case_study_name = "formose-inspired crn",
    ...
    directed_hypergraph = formose_dh,
...
println("\nprocessed formose case study data keys:")
println(keys(formose_case_study_data))

println("\nsummary:")
println("case study: ", formose_case_study_data.case_study_name)
...
println("package directed hypergraph type: ", typeof(formose_dh))